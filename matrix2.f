c     Matrix Addition and Substraction
      program matrix_add_sub
         ...
         ! open input file
         open(unit=1, file='matrix2_input.txt')
         open(unit=2, file='matrix2_output.txt')


         !write(*,*) "Enter the dimension of the matrix: "
         read(1,*) m,n
         write(2,10) "m = ",m, "n = ",n
  10     format(A,I2,3X,A,I2)
        ! Read the matrix a
        ! write(*,*) "Enter the matrix a: "
         ...
         do i=1,m
            write(2,'(10f7.2)') (b(i,j),j=1,n)
         end do

         ! add the matrix a and b
         do i=1,m
            do j = 1,n
                 c(i,j) = a(i,j) + b(i,j)
            end do 
